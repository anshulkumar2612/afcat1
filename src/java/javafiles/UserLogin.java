            ...
            out.println("<body>");
                ...
                else
                {
                    ...
                    if(rowCount==0)
                    {
                        ...
                        session.setAttribute("Message1","Invalid Credentials");
                        response.sendRedirect("CandidateLogin.jsp");
                        //request.getRequestDispatcher("CandidateLogin.jsp").include(request, response);
                    }
                    else
                    {
                        str1="insert into loginbit (completed,uid) values(?,?)";
                        ps1=con.prepareStatement(str1);
                        ps1.setInt(1, completed);
                        ps1.setInt(2, st);
                        ps1.execute();
                        session.setAttribute("Message1","Login Successful");
                        response.sendRedirect("CandidateLogin.jsp");  
                    }
